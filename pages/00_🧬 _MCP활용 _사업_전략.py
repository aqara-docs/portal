...
def debug_env_vars():
    ...
    # 비밀번호는 마스킹 처리
    if env_vars['SQL_PASSWORD']:
        env_vars['SQL_PASSWORD'] = '*' * len(env_vars['SQL_PASSWORD'])
    if env_vars['OPENAI_API_KEY']:
        env_vars['OPENAI_API_KEY'] = env_vars['OPENAI_API_KEY'][:10] + '...'
    if env_vars['ANTHROPIC_API_KEY']:
        env_vars['ANTHROPIC_API_KEY'] = env_vars['ANTHROPIC_API_KEY'][:10] + '...'
...
def load_config_from_json():
    ...
    MySQL MCP 서버의 경우 환경 변수를 사용하여 동적으로 설정을 생성합니다.
    ...
    try:
        if os.path.exists(CONFIG_FILE_PATH):
            with open(CONFIG_FILE_PATH, "r", encoding="utf-8") as f:
                ...
                # MySQL MCP 서버 설정이 있는 경우 환경 변수로 치환
                if "mcp-mysql-server" in config:
                    mysql_config = config["mcp-mysql-server"]
                    if "args" in mysql_config:
                        # 환경 변수 값 가져오기
                        ...
                        # MySQL 설정 JSON 생성
                        mysql_config_json = json.dumps({
                            "mysqlHost": sql_host,
                            "mysqlPort": int(sql_port),
                            "mysqlUser": sql_user,
                            "mysqlPassword": sql_password,
                            "mysqlDatabase": sql_database
                        })
                        
                        # args 배열에서 환경 변수 치환
                        new_args = []
                        for arg in mysql_config["args"]:
                            if arg == "${MYSQL_CONFIG}":
                                new_args.append(mysql_config_json)
                            else:
                                new_args.append(arg)
                        
                        config["mcp-mysql-server"]["args"] = new_args
...
with st.sidebar:
    ...
    # 환경 변수 디버깅 정보 표시
    with st.expander("🔧 환경 변수 디버깅", expanded=False):
        env_vars = debug_env_vars()
        st.json(env_vars)
        
        # MySQL 연결 테스트 버튼
        if st.button("🔗 MySQL 연결 테스트", type="secondary"):
            try:
                conn = connect_to_db()
                if conn:
                    st.success("✅ MySQL 연결 성공!")
                    conn.close()
                else:
                    st.error("❌ MySQL 연결 실패!")
            except Exception as e:
                st.error(f"❌ MySQL 연결 오류: {str(e)}")
...
def init_database():
    ...
    try:
        ...
        # mcp_analysis_results 테이블 생성
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS mcp_analysis_results (
        ...
        conn.commit()
        return True, "데이터베이스 초기화가 완료되었습니다."
        
    except Exception as e:
        if 'conn' in locals():
            conn.rollback()
        return False, f"데이터베이스 초기화 중 오류 발생: {str(e)}"
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
