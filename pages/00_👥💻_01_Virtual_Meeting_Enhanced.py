...
모든 분들께서는 지금까지의 논의 내용을 바탕으로 간결하고 명확하게 의견을 정리해 주시기 바랍니다. 특히 마지막 라운드에서는 반드시 회의를 마무리해 주세요."""
                ...
                # 🎯 연장 1라운드 완료 - 마지막 라운드 공지
                if not hasattr(self, 'final_round_announced'):
                    moderator = self.get_moderator()
                    if moderator:
                        final_round_announcement = f"""
🏁 **연장 1라운드가 완료되었습니다**
...
오늘 회의에서 나온 주요 논점들과 각자의 의견이 충분히 공유되었습니다. 
모든 참가자분들께서 자신의 발언 히스토리를 정리하고 감사 인사까지 해주셔서 완전한 마무리가 되었습니다.
...
모든 분들의 적극적인 참여에 감사드립니다."""
    ...
    def _is_meeting_properly_concluded(self) -> bool:
        """회의가 제대로 마무리되었는지 종합적으로 판단 - 10라운드 완료 시 사용"""
        if len(self.messages) < 10:  # 최소 메시지 수
            return False
        ...
        if len(last_round_messages) < len(non_moderator_personas):  # 마지막 라운드가 완료되지 않음
            return False
        
        # 2. 마무리 키워드 분석 (더 엄격한 기준)
        conclusion_keywords = ['결론', '마무리', '정리', '최종', '감사', '인사', '끝', '완료', '종료']
        summary_keywords = ['요약', '정리하면', '종합하면', '전체적으로', '결국', '따라서']
        gratitude_keywords = ['감사', '고생', '수고', '참여', '협력']
        
        conclusion_count = 0
        summary_count = 0
        gratitude_count = 0
        
        # 마지막 라운드 메시지들에서 마무리 패턴 확인
        for msg in last_round_messages:
            ...
            # 감사/인사 키워드 체크
            if any(keyword in content_lower for keyword in gratitude_keywords):
                gratitude_count += 1
        
        # 3. 종합 판단 기준
        total_participants = len(non_moderator_personas)
        
        # 최소 50% 이상이 마무리 의도를 보이고, 30% 이상이 감사 인사를 했으면 마무리된 것으로 판단
        conclusion_ratio = conclusion_count / total_participants
        summary_ratio = summary_count / total_participants
        gratitude_ratio = gratitude_count / total_participants
        
        # 마무리 조건:
        # 1) 50% 이상이 결론/마무리 키워드 사용 AND
        # 2) 30% 이상이 요약/정리 키워드 사용 AND  
        # 3) 30% 이상이 감사/인사 키워드 사용
        is_properly_concluded = (
            conclusion_ratio >= 0.5 and 
            summary_ratio >= 0.3 and 
            gratitude_ratio >= 0.3
        )
        
        # 4. 추가 검증: 메시지 길이 (마무리 메시지는 보통 길다)
        avg_last_round_length = sum(len(msg.content) for msg in last_round_messages) / len(last_round_messages)
        overall_avg_length = sum(len(msg.content) for msg in self.messages if not msg.is_moderator) / len([msg for msg in self.messages if not msg.is_moderator])
        
        # 마지막 라운드 메시지가 평균보다 20% 이상 길면 마무리 의도가 있다고 판단
        length_indicates_conclusion = avg_last_round_length > (overall_avg_length * 1.2)
        
        return is_properly_concluded and length_indicates_conclusion
...
def validate_final_statement(statement: str, persona_name: str) -> Dict[str, bool]:
    ...
    # 1. 개인 발언 히스토리 요약 체크
    personal_indicators = ['제가 이번 회의에서', '제가 강조한', '제가 제시한', '저의 핵심', '제 의견']
    validation_result['has_personal_summary'] = any(indicator in statement for indicator in personal_indicators)
    
    # 2. 전체 결론 도출 체크
    conclusion_indicators = ['전체 논의를 종합', '결론적으로', '종합해보면', '전반적으로', '최종적으로']
    validation_result['has_overall_conclusion'] = any(indicator in statement for indicator in conclusion_indicators)
    
    # 3. 실행 방안 체크
    action_indicators = ['실행 방안', '다음 단계', '액션 플랜', '구체적으로', '실행하', '진행하', '추진하']
    validation_result['has_action_plan'] = any(indicator in statement for indicator in action_indicators)
    
    # 4. 다른 참가자와의 연결점 체크 (참가자 이름이나 의견 언급)
    connection_indicators = ['님의', '분의', '동의하', '공감하', '함께', '합의']
    validation_result['has_participant_connection'] = any(indicator in statement for indicator in connection_indicators)
    
    # 5. 감사 인사 체크
    gratitude_indicators = ['감사', '고생', '수고', '고마', '함께해', '동참']
    validation_result['has_gratitude'] = any(indicator in statement for indicator in gratitude_indicators)
    
    # 전체 완성도 체크
    validation_result['is_complete'] = all([
...
def generate_ai_response(persona: Persona, conversation_history: str, meeting_topic: str, file_content: str, round_number: int, enhanced_context: str = "", model_name: str = "gpt-4o-mini", total_rounds: int = 10, is_final_round: bool = False, final_round_context: str = "", conversation_direction: str = "", meeting: 'VirtualMeeting' = None) -> str:
    ...
    try:
        ...
        if is_final_extension_round:
            ...
            1️⃣ **자신의 발언 히스토리 요약** 
               - "제가 이번 회의에서 강조한 핵심 포인트는..."
               - 자신이 제시한 주요 의견 2-3개를 구체적으로 언급
            
            2️⃣ **회의 전체 결론 도출**
               - "전체 논의를 종합해보면..."
               - 회의에서 도달한 최종 결론이나 합의사항
            
            3️⃣ **구체적 실행 방안 제시**
               - "구체적인 실행 방안으로는..." 또는 "다음 단계로..."
               - 실제 실행 가능한 액션 플랜이나 후속 조치
            ...
            5️⃣ **감사 인사 (필수)**
               ...
               - 또는 "고생 많으셨습니다" 등의 마무리 인사
            
            ⚠️ **경고**: 위 5단계 중 하나라도 빠뜨리면 불완전한 마무리로 간주됩니다.
            
            💡 **발언 예시 템플릿**:
            "제가 이번 회의에서 강조한 핵심은 [구체적 내용1], [구체적 내용2], [구체적 내용3]이었습니다. 
            전체 논의를 종합해보면 [최종 결론]에 도달했다고 생각합니다. 
            구체적인 실행 방안으로는 [실행방안1]과 [실행방안2]를 제안합니다. 
            특히 [참가자명]님의 [구체적 의견]에 깊이 공감하며, [합의점]에서 함께 나아갈 수 있을 것 같습니다. 
            오늘 모든 분들과 함께 의미 있는 논의를 할 수 있어 정말 감사했습니다."
            
            🎯 **이번이 당신의 마지막 발언입니다. 반드시 완결성 있게 마무리하세요.**
            ...
            - 아직 해결되지 않은 쟁점이 있다면 언급하세요
            - 다음 라운드(최종 라운드)를 위한 방향을 제시하세요
        ...
        if model_name.startswith('claude'):
            ...
            # 🎯 마무리 라운드 발언 품질 검증 및 개선
            if is_final_extension_round or (is_final_round and round_number >= total_rounds):
                validation = validate_final_statement(ai_response, persona.name)
                
                if not validation['is_complete']:
                    # 불완전한 마무리 발언인 경우 개선 시도
