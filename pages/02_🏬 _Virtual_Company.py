...
def search_all_rag_sources_efficiently(keyword, max_items_per_source=30):
    ...
    try:
        ...
        if hasattr(st.session_state, 'mysql_data') and st.session_state.mysql_data:
            for table_name, df in st.session_state.mysql_data.items():
                ...
                for idx, row in df.iterrows():
                    if search_count >= max_items_per_source:  # 비용 절약: 제한
                        break
                    ...
                    if keyword.lower() in row_text.lower():
                        all_related_content.append({
                            ...
                            'row_id': idx,
                            'content': row_text,
                            'relevance_score': row_text.lower().count(keyword.lower())
                        })
                        search_count += 1
        
        # 2. 웹사이트 데이터 검색
        if hasattr(st.session_state, 'website_data') and st.session_state.website_data:
            for i, page_data in enumerate(st.session_state.website_data[:max_items_per_source]):  # 비용 절약: 제한
        ...
        if hasattr(st.session_state, 'files_data') and st.session_state.files_data:
            for file_info in st.session_state.files_data[:max_items_per_source]:  # 비용 절약: 제한
        ...
        # 관련성 순으로 정렬 (비용 절약: 상위 결과만 사용)
        all_related_content.sort(key=lambda x: x['relevance_score'], reverse=True)
        return all_related_content[:max_items_per_source]  # 최대 30개로 제한
        
    except Exception as e:
        st.warning(f"RAG 검색 중 오류: {e}")
        return []

def chunk_rag_content_efficiently(content_list, max_tokens_per_chunk=1500):
    """비용 절약을 위한 효율적 청크 분할 (매우 작은 청크 사이즈)"""
    chunks = []
    current_chunk = []
    current_tokens = 0
    ...
    for item in content_list:
        ...
        if tokens > max_tokens_per_chunk:
            ...
            for sentence in sentences:
                sentence_tokens = estimate_token_count_efficient(sentence)
...
# ================= 기본 RAG 컨텍스트 생성 함수 (호환성 유지) =================

def create_rag_context():
    """RAG 컨텍스트 생성 (비용 절약 최적화)"""
    context_parts = []
    rag_sources_used = []
    ...
    if st.session_state.mysql_data:
        ...
        for table_name, df in st.session_state.mysql_data.items():
            ...
            # 샘플 데이터 (상위 2행만) - 비용 절약 최적화
            if len(df) > 0:
                mysql_context += "- 샘플 데이터:\n"
                mysql_context += df.head(2).to_string(index=False)[:500] + "...\n"  # 500자로 축소
...
def run_concurrent_analysis_with_progress_rag(user_query, persona_prompts, persona_status, persona_progress, model_name="claude-3-7-sonnet-latest", active_personas=None):
    """RAG 기능이 포함된 동시 분석 실행 (선택된 페르소나만)"""
    
    # 활성 페르소나 설정 (기본값은 모든 페르소나)
    if active_personas is None:
        active_personas = PERSONAS
    
    # 분석 작업 준비 (선택된 페르소나만)
    tasks = []
    for persona_key, persona_info in active_personas.items():
        custom_prompt = persona_prompts.get(persona_key, "")
        tasks.append((user_query, persona_key, persona_info, custom_prompt, model_name))
    
    # 각 페르소나별 진행 상태 추적 (선택된 페르소나만)
    persona_stages = {}
    for persona_key in active_personas.keys():
...
persona_prompts = {}
# 모든 페르소나에 대해 커스터마이징 옵션 제공 (나중에 필터링됨)
...
# 💰 비용 절약 + 완전한 RAG 옵션
st.markdown("---")
st.markdown("### 💰 비용 절약 + 고급 RAG 설정")

cost_col1, cost_col2, cost_col3 = st.columns(3)

with cost_col1:
    # 비용 절약 모드 활성화
    use_cost_optimized_rag = st.checkbox(
        "💰 비용 절약 진정한 RAG 사용", 
        value=False,
        help="전체 RAG 데이터를 검색하고 점진적 요약하지만, 청크 수와 토큰을 제한하여 비용을 대폭 절약합니다"
    )
    
    if use_cost_optimized_rag:
        max_chunks = st.slider("처리할 청크 수", 3, 12, 8, help="더 적은 청크 = 더 많은 비용 절약")

with cost_col2:
    # 페르소나 선택
    use_selective_personas = st.checkbox(
        "👥 선택적 페르소나 실행",
        value=False,
        help="필요한 페르소나만 선택하여 분석 비용을 절약합니다"
    )
    
    if use_selective_personas:
        selected_personas = st.multiselect(
            "분석할 페르소나 선택",
            options=list(PERSONAS.keys()),
            default=list(PERSONAS.keys())[:3],  # 기본으로 3개만
            help="비용 절약을 위해 필요한 페르소나만 선택하세요"
        )

with cost_col3:
    # 모델 최적화
    use_cost_optimized_model = st.checkbox(
        "🧠 비용 최적화 모델 사용",
        value=False,
        help="더 저렴한 AI 모델을 자동으로 선택하여 비용을 절약합니다"
    )
    
    if use_cost_optimized_model:
        st.info("💡 GPT-4 → GPT-4o-mini\nClaude Sonnet → Claude Haiku")

# 비용 절약 모드 활성화 시 예상 절약 효과 표시
if use_cost_optimized_rag or use_selective_personas or use_cost_optimized_model:
    with st.expander("💰 예상 비용 절약 효과", expanded=True):
        savings = []
        if use_cost_optimized_rag:
            max_chunks_val = max_chunks if use_cost_optimized_rag else 8
            savings.append(f"🔍 RAG 청크 제한: ~{(20-max_chunks_val) * 1500:,} 토큰 절약")
        if use_selective_personas:
            selected_count = len(selected_personas) if use_selective_personas else 6
            savings.append(f"👥 페르소나 선택: {6-selected_count}개 페르소나 생략")
        if use_cost_optimized_model:
            savings.append("🧠 모델 최적화: ~70% 모델 비용 절약")
        
        total_estimated_savings = 0
        if use_cost_optimized_rag: total_estimated_savings += 60
        if use_selective_personas: total_estimated_savings += (6-len(selected_personas if use_selective_personas else [1,2,3,4,5,6])) * 10
        if use_cost_optimized_model: total_estimated_savings += 70
        
        st.success(f"🎯 **총 예상 비용 절약: ~{min(total_estimated_savings, 90)}%**")
...
if st.button("🚀 C-Level 임원진 분석 시작", type="primary", use_container_width=True, key="main_analysis_start"):
    if not user_query.strip():
        ...
        # 💰 비용 절약 진정한 RAG 처리
        if use_cost_optimized_rag and st.session_state.rag_sources:
            st.markdown("---")
            st.markdown("## 💰 비용 절약 진정한 RAG 분석")
            
            # 키워드 추출 (간단한 방식)
            keywords = [word.strip() for word in user_query.split() if len(word) > 2][:3]  # 상위 3개 키워드
            
            for keyword in keywords:
                st.markdown(f"### 🔍 '{keyword}' 관련 RAG 분석")
                rag_result = true_rag_cost_optimized(
                    keyword, 
                    selected_model if not use_cost_optimized_model else get_cost_optimized_model(selected_model), 
                    max_chunks if use_cost_optimized_rag else 8
                )
                
                if rag_result['final_summary']:
                    st.markdown("#### 📊 RAG 분석 결과")
                    st.markdown(rag_result['final_summary'])
                    
                    # 간단한 통계 표시
                    st.info(f"💰 처리된 청크: {rag_result['total_chunks']}개 | 분석된 항목: {rag_result['total_content_items']}개")
        
        # 💰 비용 최적화 모델 설정
        analysis_model = selected_model
        if use_cost_optimized_model:
            analysis_model = get_cost_optimized_model(selected_model)
            st.info(f"💰 비용 최적화: {selected_model} → {analysis_model}")
        
        # 💰 선택적 페르소나 설정
        active_personas = PERSONAS
        if use_selective_personas and selected_personas:
            active_personas = {k: v for k, v in PERSONAS.items() if k in selected_personas}
            st.info(f"👥 선택된 페르소나: {len(active_personas)}/{len(PERSONAS)}개")
        
        # 기존 전체 분석 플로우(동시 페르소나 분석 등) 실행
        # 실행 시간 측정 시작
        ...
        st.session_state.selected_model = analysis_model  # 최적화된 모델명 저장
        # 진행 상황 표시
        ...
        with progress_container:
            ...
            # 동시 분석 실행 (선택된 페르소나만)
            with st.spinner(f"🔥 {len(active_personas)}명의 C-Level 임원진이 동시에 분석 중입니다..."):
                # 선택된 페르소나에 대한 프롬프트만 추출
                filtered_persona_prompts = {k: persona_prompts.get(k, "") for k in active_personas.keys()}
                # 실제 동시 분석 수행 (선택된 페르소나만)
                analysis_results = run_concurrent_analysis_with_progress_rag(
                    user_query, filtered_persona_prompts, persona_status, persona_progress, analysis_model, active_personas
                )
                # 결과를 세션에 저장
                ...
                for persona_key, result_data in analysis_results.items():
                    persona_info = active_personas[persona_key]  # active_personas 사용
            ...
            if successful_analyses:
                ...
                with st.spinner("CEO 최종 종합 분석 중..."):
                    ceo_synthesis = synthesize_ceo_analysis_rag(user_query, successful_analyses, analysis_model)
                    # 실행 시간 계산
                    ...
                    if ceo_synthesis:
                        ...
                        with st.spinner("📊 분석 결과를 자동으로 저장하고 있습니다..."):
                            analysis_id = save_virtual_company_analysis_new(
                                ...
                                model_name=analysis_model,  # 비용 최적화 모델 사용
