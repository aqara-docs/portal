...
# 페이지 설정
st.set_page_config(
    page_title="Virtual Meeting Auto Fixed",
    page_icon="🎭",
    layout="wide",
...
class VirtualMeeting:
    def __init__(self):
        ...
        self.next_auto_time: Optional[datetime] = None
    ...
    def add_message(self, persona_id: str, content: str, is_human_input: bool = False) -> Message:
        ...
        if persona:
            ...
            # 메시지 추가 후 다음 자동 시간 설정
            if self.auto_mode:
                self.next_auto_time = datetime.now() + timedelta(seconds=self.speaking_speed)
            return message
        return None
    ...
    def should_auto_execute(self) -> bool:
        ...
        if not self.next_auto_time:
            return True  # 첫 실행
        
        return datetime.now() >= self.next_auto_time
    ...
    def get_time_until_next(self) -> float:
        ...
        remaining = (self.next_auto_time - datetime.now()).total_seconds()
        return max(0.0, remaining)

def initialize_session_state():
    """세션 상태 초기화"""
    if 'virtual_meeting_auto_fixed' not in st.session_state:
        st.session_state.virtual_meeting_auto_fixed = VirtualMeeting()
        ...
        st.session_state.virtual_meeting_auto_fixed.add_persona(moderator)
...
def stream_response(text: str, delay: float = 0.03) -> Generator[str, None, None]:
    """텍스트를 단어별로 스트리밍"""
    words = text.split()
...
def execute_conversation_round(meeting: VirtualMeeting) -> bool:
    """대화 라운드 실행"""
...
def main():
    st.title("🎭 Virtual Meeting Auto Fixed - 자동 진행 수정 버전")
    
    # 세션 상태 초기화
    initialize_session_state()
    meeting = st.session_state.virtual_meeting_auto_fixed
    
    # 페이지 최상단에서 자동 실행 체크
    auto_executed = False
    if meeting.auto_mode and meeting.is_active and meeting.should_auto_execute():
        if execute_conversation_round(meeting):
            auto_executed = True
        else:
            # 회의 자동 종료
        ...
        if not meeting.is_active:
            ...
            if new_auto_mode != meeting.auto_mode:
                ...
                if meeting.auto_mode:
                    # 자동 모드 시작 시 즉시 첫 실행 준비
                    meeting.next_auto_time = datetime.now()
    ...
    if not meeting.is_active:
        ...
        # 자동 모드 상태 표시
        if meeting.auto_mode:
            time_left = meeting.get_time_until_next()
            if time_left > 0:
                progress = 1.0 - (time_left / meeting.speaking_speed)
                st.progress(progress, text=f"🤖 다음 발언까지 {time_left:.1f}초 남음")
                # 자동 실행 후에는 즉시 새로고침
                if auto_executed:
                    st.rerun()
                # 일반적인 경우 1초마다 새로고침
