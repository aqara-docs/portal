...
class FileRAGSystem:
    ...
    # FileRAGSystem 클래스의 _setup_embeddings 메서드 수정
    ...
    def test_rag_system(self):
        ...
        test_info = {
            ...
            'docs_count': len(self.docs) if hasattr(self, 'docs') else 0,
            'vectorstore_exists': hasattr(self, 'vectorstore') and self.vectorstore is not None,
            'current_folder': self.current_folder_path
        }
        
        if self.is_loaded and hasattr(self, 'docs') and self.docs:
            # 간단한 테스트 검색
            try:
                test_results = self.search("테스트", k=1)
                test_info['search_works'] = len(test_results) > 0
            except Exception as e:
                test_info['search_works'] = False
                test_info['search_error'] = str(e)
        else:
            test_info['search_works'] = False
    ...
    def search(self, query, k=5):
        ...
        try:
            ...
            # 5. hybrid_score 계산 및 정렬
            results = list(all_results.values())
            for doc in results:
                doc.metadata['hybrid_score'] = doc.metadata.get('search_score', 0) + doc.metadata.get('keyword_score', 0) + doc.metadata.get('graph_score', 0)
            results.sort(key=lambda x: x.metadata.get('hybrid_score', 0), reverse=True)
            return results[:k]
        except Exception as e:
            st.error(f"Graph RAG 검색 오류: {e}")
            return []
    
    def _get_dynamic_k(self, query, base_k):
        """쿼리와 문서 유형에 따른 동적 k값 계산 (단순화)"""
        # 기본 k값 사용 (복잡한 로직 제거)
        return base_k
    ...
    def _analyze_question_type(self, query):
        ...
        # 담당자/책임자 질문
        if any(keyword in query_lower for keyword in ["담당", "역할","책임", "누가", "담당자", "책임자"]):
...
class FileRAGChatbot:
    ...
    def generate_response(self, user_query, provider='openai', model=None, temperature=0.7):
        try:
            ...
            # 1. RAG를 통한 관련 문서 검색 (문서 유형에 따른 동적 k값 사용)
            dynamic_k = self._get_dynamic_k_for_chatbot(user_query)
            relevant_docs = self.rag_system.search(user_query, k=dynamic_k)
    ...
    def _get_dynamic_k_for_chatbot(self, user_query):
        """일반 챗봇에서 문서 유형에 따른 동적 k값 계산 (단순화)"""
        # 기본 k값 사용 (복잡한 로직 제거)
        return 5
...
class MultiModalProcessor:
    ...
    def __init__(self):
        ...
        self.supported_document_types = ['.pdf', '.docx', '.doc', '.txt', '.md']
        self.supported_excel_types = ['.xlsx', '.xls', '.csv']
        self.supported_presentation_types = ['.pptx', '.ppt']
        
    def process_uploaded_file(self, uploaded_file):
        """업로드된 파일을 처리하고 내용을 추출"""
        try:
            # 파일 이름이 없는 경우 처리
            ...
            if file_extension in self.supported_image_types:
                ...
                return self._process_document(uploaded_file)
            elif file_extension in self.supported_excel_types:
                return self._process_excel(uploaded_file)
            elif file_extension in self.supported_presentation_types:
                return self._process_presentation(uploaded_file)
            else:
                return {
                    'type': 'error',
            ...
            return {
                'type': 'error',
                'content': f'파일 처리 중 오류 발생: {str(e)}',
                'metadata': {'filename': getattr(uploaded_file, 'name', 'unknown')}
            }
    ...
    def _process_image(self, uploaded_file):
        ...
        try:
            ...
            return {
                'type': 'error',
                'content': f'이미지 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }
    
    def _process_document(self, uploaded_file):
        """문서 파일 처리"""
        try:
            file_extension = os.path.splitext(uploaded_file.name)[1].lower()
            ...
            if file_extension == '.pdf':
                ...
                return {
                    'type': 'error',
                    'content': f'지원하지 않는 문서 형식: {file_extension}',
                    'metadata': {'filename': uploaded_file.name}
                }
        except Exception as e:
            return {
                'type': 'error',
                'content': f'문서 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }
    ...
    def _process_word(self, uploaded_file):
        ...
        try:
            ...
            return {
                'type': 'error',
                'content': f'Word 문서 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }
    
    def _process_text(self, uploaded_file):
        """텍스트 파일 처리"""
        try:
            # 파일 포인터를 처음으로 되돌림
            uploaded_file.seek(0)
            ...
            return {
                'type': 'error',
                'content': f'텍스트 파일 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }
    ...
    def _process_excel(self, uploaded_file):
        ...
        try:
            ...
            content = f"Excel 파일: {uploaded_file.name}\n\n"
            
            for sheet_name, df in excel_data.items():
                content += f"--- 시트: {sheet_name} ---\n"
                content += f"행 수: {len(df)}, 열 수: {len(df.columns)}\n"
                content += f"컬럼: {list(df.columns)}\n\n"
            ...
            return {
                'type': 'error',
                'content': f'Excel 파일 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }
    
    def _process_presentation(self, uploaded_file):
        """프레젠테이션 파일 처리"""
        try:
            ...
            content = f"프레젠테이션: {uploaded_file.name}\n\n"
            ...
            return {
                'type': 'error',
                'content': f'프레젠테이션 처리 중 오류: {str(e)}',
                'metadata': {'filename': uploaded_file.name}
            }

# ===== 멀티모달 챗봇 클래스 =====
class MultiModalChatbot(FileRAGChatbot):
    """멀티모달 기능이 추가된 챗봇 클래스"""
    ...
    def add_uploaded_file(self, uploaded_file):
        """업로드된 파일을 처리하고 추가"""
        try:
            processed_file = self.multimodal_processor.process_uploaded_file(uploaded_file)
            self.uploaded_files.append(processed_file)
            return processed_file
        except Exception as e:
            # 파일 처리 중 오류가 발생한 경우
            error_file = {
                'type': 'error',
                'content': f'파일 처리 중 오류: {str(e)}',
                'metadata': {'filename': getattr(uploaded_file, 'name', 'unknown')}
            }
            self.uploaded_files.append(error_file)
            return error_file
    ...
    def generate_multimodal_response(self, user_query, provider='openai', model=None, temperature=0.7):
        """멀티모달 응답 생성"""
        try:
            # 1. 업로드된 파일 정보를 포함한 컨텍스트 구성
            file_context = self._build_file_context()
            
            # 디버깅: 파일 컨텍스트 확인
            if not file_context:
                return "업로드된 파일이 없거나 파일 처리에 실패했습니다.", None
            
            # 2. RAG 검색 (문서 유형에 따른 동적 k값 사용)
            dynamic_k = self._get_multimodal_k(user_query)
            relevant_docs = self.rag_system.search(user_query, k=dynamic_k)
            
            # 3. 파일 컨텍스트와 RAG 결과를 결합
            combined_context = self._combine_contexts(file_context, relevant_docs)
            
            # 4. 멀티모달 프롬프트 구성
            messages = self._build_multimodal_prompt(user_query, combined_context)
            ...
            self.conversation_history.append({
                ...
                'provider': provider,
                'model': model,
                'uploaded_files': [f['metadata']['filename'] for f in self.uploaded_files]
            })
            ...
            import traceback
            error_msg = f"멀티모달 응답 생성 오류: {str(e)}\n상세: {traceback.format_exc()}"
            return error_msg, None
    
    def _get_multimodal_k(self, user_query):
        """멀티모달 모드에서 문서 유형에 따른 동적 k값 계산 (단순화)"""
        # 기본 k값 사용 (복잡한 로직 제거)
        return 5
    ...
    def _build_file_context(self):
        ...
        context_parts = ["📁 **업로드된 파일 분석 정보:**"]
        
        for i, file_info in enumerate(self.uploaded_files, 1):
            if file_info['type'] == 'error':
                context_parts.append(f"{i}. ❌ {file_info['content']}")
            elif file_info['type'] == 'image':
                context_parts.append(f"{i}. 🖼️ {file_info['metadata']['filename']}")
                context_parts.append(f"   - 형식: {file_info['metadata']['format']}")
                context_parts.append(f"   - 크기: {file_info['metadata']['size']}")
                context_parts.append(f"   - 색상 모드: {file_info['metadata']['mode']}")
                context_parts.append(f"   - 이미지 설명: {file_info['content']}")
            elif file_info['type'] == 'document':
                context_parts.append(f"{i}. 📄 {file_info['metadata']['filename']}")
                context_parts.append(f"   - 형식: {file_info['metadata']['file_type']}")
                if 'pages' in file_info['metadata']:
                    context_parts.append(f"   - 페이지: {file_info['metadata']['pages']}")
                elif 'paragraphs' in file_info['metadata']:
                    context_parts.append(f"   - 단락: {file_info['metadata']['paragraphs']}")
                elif 'sheets' in file_info['metadata']:
                    context_parts.append(f"   - 시트: {', '.join(file_info['metadata']['sheets'])}")
                elif 'slides' in file_info['metadata']:
                    context_parts.append(f"   - 슬라이드: {file_info['metadata']['slides']}")
                
                # 문서 전체 내용 제공 (분석을 위해 더 많은 내용 포함)
                content = file_info['content']
                if len(content) > 2000:
                    content = content[:2000] + "\n\n[내용이 길어서 일부만 표시됩니다. 전체 내용을 바탕으로 분석해주세요.]"
                context_parts.append(f"   - 문서 내용:\n{content}")
        
        return "\n\n".join(context_parts)
    
    def _combine_contexts(self, file_context, relevant_docs):
        """파일 컨텍스트와 RAG 결과를 결합"""
        combined = []
        
        if file_context:
            combined.append(file_context)
    ...
    def _build_multimodal_prompt(self, user_query, context):
        """멀티모달 프롬프트 구성"""
        system_prompt = f"""당신은 아카라라이프의 멀티모달 챗봇입니다.

【답변 규칙】
1. 사용자가 업로드한 파일의 내용을 정확히 분석하고 답변하세요.
2. 이미지 파일이 있는 경우: 이미지의 시각적 요소, 텍스트, 구조, 의미를 상세히 분석하세요.
3. 문서 파일이 있는 경우: 문서의 주요 내용, 구조, 핵심 정보를 요약하고 분석하세요.
4. 스프레드시트가 있는 경우: 데이터 구조, 주요 수치, 트렌드를 분석하세요.
5. 프레젠테이션이 있는 경우: 슬라이드 내용, 구조, 핵심 메시지를 분석하세요.
6. 파일 내용을 바탕으로 사용자 질문에 구체적이고 유용한 답변을 제공하세요.
7. 파일에 없는 정보는 '해당 정보가 파일에 없습니다'라고 명확히 답변하세요.
8. 한국어로 친근하고 명확하게 답변하세요.
...
위 파일 내용을 바탕으로 사용자의 질문에 답변해주세요. 파일의 구체적인 내용을 인용하고 분석하여 답변하세요."""

        return [{"role": "system", "content": system_prompt}]

# ===== 멀티모달 파일 분석 함수 =====
def analyze_uploaded_files(uploaded_files, user_query, provider='openai', model=None):
    """업로드된 파일들을 분석하는 함수"""
    try:
        ...
        for uploaded_file in uploaded_files:
            chatbot.add_uploaded_file(uploaded_file)
        
        # 멀티모달 응답 생성
        response, relevant_docs = chatbot.generate_multimodal_response(
            user_query, provider, model
        )
        
        return response, relevant_docs, chatbot.uploaded_files
        
    except Exception as e:
        return f"파일 분석 중 오류: {str(e)}", None, []
...
if 'conversations' not in st.session_state:
    ...
    st.session_state.conversations['멀티모달'] = []
...
for i, message in enumerate(current_conversation):
    if message['role'] == '사용자':
        ...
        if message.get('persona_analyses') and selected_folder == "재무 정보-고급":
            ...
            if selected_folder == "일반 채팅":
                # 멀티모달 대화인 경우
                if 'uploaded_files' in message and message['uploaded_files']:
                    st.markdown(f"<div class='bot-bubble'>🤖 {message['content']}</div>", unsafe_allow_html=True)
                    st.info(f"📁 분석된 파일: {', '.join(message['uploaded_files'])}")
                else:
...
# 멀티모달 파일 업로드 섹션
st.markdown("#### 📁 파일 업로드 (멀티모달)")
uploaded_files = st.file_uploader(
    "분석할 파일을 업로드하세요:",
    type=['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'pdf', 'docx', 'doc', 'txt', 'md', 'xlsx', 'xls', 'csv', 'pptx', 'ppt'],
    accept_multiple_files=True,
    help="이미지, PDF, Word, Excel, PowerPoint, 텍스트 파일을 지원합니다.",
    key="multimodal_file_uploader"
)

# 업로드된 파일 처리
if uploaded_files:
    # 파일 정보를 세션 상태에 저장 (파일 객체 대신 메타데이터 저장)
    if 'uploaded_files_info' not in st.session_state:
        st.session_state.uploaded_files_info = []
    
    # 새로운 파일이 업로드된 경우에만 정보 업데이트
    current_files = [f.name for f in uploaded_files]
    if not st.session_state.uploaded_files_info or [f['name'] for f in st.session_state.uploaded_files_info] != current_files:
        st.session_state.uploaded_files_info = []
        for file in uploaded_files:
            file_info = {
                ...
                'type': file.type,
                'size': file.size,
                'data': file.read()  # 파일 데이터를 메모리에 저장
            }
            st.session_state.uploaded_files_info.append(file_info)
            file.seek(0)  # 파일 포인터를 처음으로 되돌림
        st.success(f"✅ 파일 정보가 세션에 저장되었습니다: {len(st.session_state.uploaded_files_info)}개 파일")
    
    st.success(f"✅ {len(uploaded_files)}개 파일이 업로드되었습니다. 멀티모달 분석이 활성화되었습니다.")
    
    # 파일 목록 표시
    for i, file in enumerate(uploaded_files, 1):
        file_extension = os.path.splitext(file.name)[1].lower()
        if file_extension in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp']:
            st.markdown(f"{i}. 🖼️ {file.name}")
    ...
    # 멀티모달 모드 안내
    st.info("🤖 멀티모달 모드가 활성화되었습니다. 파일 내용을 분석하여 답변합니다.")

# 파일 초기화 버튼
if st.session_state.get('uploaded_files_info') and st.button("🗑️ 업로드된 파일 초기화", key="clear_files_btn"):
    st.session_state.uploaded_files_info = []
    st.session_state.multimodal_chatbot.clear_uploaded_files()
    st.success("업로드된 파일이 초기화되었습니다.")
    st.rerun()
...
# 입력 필드 (멀티모달 모드에서는 별도 키 사용)
if st.session_state.get('uploaded_files_info'):
    # 멀티모달 모드: 파일이 업로드된 경우
    if 'multimodal_input_reset' not in st.session_state:
        st.session_state['multimodal_input_reset'] = 0
    input_key = f"multimodal_input_{st.session_state['multimodal_input_reset']}"
else:
    # 일반 모드: 파일이 없는 경우
...
user_input = st.text_area(
    ...
    placeholder="메시지를 입력하세요... (파일을 업로드하면 멀티모달 분석이 가능합니다)"
)

if st.button("전송", type="primary", key="send_button"):
    if user_input.strip():
        # 멀티모달 모드 확인 (파일이 업로드된 경우)
        is_multimodal_mode = bool(st.session_state.get('uploaded_files_info'))
        
        # 현재 폴더에 따른 대화 기록 선택
        if selected_folder == "재무 정보-고급":
            current_conversation_key = '재무 정보-고급'
        elif selected_folder == "일반 채팅":
            current_conversation_key = '멀티모달' if is_multimodal_mode else '일반'
        else:
            current_conversation_key = '멀티모달' if is_multimodal_mode else '일반'
        ...
        if is_multimodal_mode:
            ...
            with st.spinner("멀티모달 분석 중입니다..."):
                try:
                    # 디버깅 정보 표시
                    st.info(f"📁 분석할 파일 수: {len(st.session_state.uploaded_files_info)}")
                    
                    # 세션에 저장된 파일 정보를 사용하여 멀티모달 챗봇에 추가
                    for i, file_info in enumerate(st.session_state.uploaded_files_info, 1):
                        st.info(f"📄 파일 {i} 처리 중: {file_info['name']}")
                        
                        # 파일 정보를 임시 파일 객체로 변환
                        import io
                        temp_file = io.BytesIO(file_info['data'])
                        temp_file.name = file_info['name']
                        temp_file.type = file_info['type']
                        temp_file.size = file_info['size']
                        
                        processed_file = st.session_state.multimodal_chatbot.add_uploaded_file(temp_file)
                        st.info(f"✅ 파일 {i} 처리 완료: {processed_file['type']}")
                    
                    # 멀티모달 응답 생성
                    st.info("🤖 멀티모달 응답 생성 중...")
                    response, relevant_docs = st.session_state.multimodal_chatbot.generate_multimodal_response(
                    ...
                    import traceback
                    st.error(f"멀티모달 분석 중 오류: {str(e)}")
                    st.error(f"상세 오류: {traceback.format_exc()}")
                    full_response = f"멀티모달 분석 중 오류가 발생했습니다: {str(e)}"
                    response_container.markdown(f"<div class='bot-bubble'>❌ {full_response}</div>", unsafe_allow_html=True)
            
            # 멀티모달 대화 기록에 저장
            st.session_state.conversations['멀티모달'].append({
                'role': '챗봇', 
                'content': full_response, 
                'timestamp': datetime.now(), 
                'provider': selected_provider, 
                'model': selected_model,
                'uploaded_files': [f['name'] for f in st.session_state.uploaded_files_info]
            })
        ...
        # 멀티모달 모드에서는 st.rerun() 대신 키 변경으로 입력 초기화
        if is_multimodal_mode:
            # 멀티모달 모드에서는 페이지 새로고침 없이 키만 변경
            if 'multimodal_input_reset' not in st.session_state:
                st.session_state['multimodal_input_reset'] = 0
            st.session_state['multimodal_input_reset'] += 1
            st.success(f"멀티모달 모드: {len(st.session_state.get('uploaded_files_info', []))}개 파일이 유지됩니다.")
        else:
            # 일반 모드에서만 페이지 새로고침
            st.rerun()
