...
def main():
    ...
    with col2:
        ...
        framework_groups = {
            "전략 분석": {
                "SWOT 분석": "기업의 강점, 약점, 기회, 위협 요인을 분석",
                "마이클 포터의 경쟁전략": "산업 내 경쟁 구조와 포지셔닝 분석",
                "협력적 경쟁관계": "경쟁사와의 협력 가능성과 시너지 분석"
            },
            "혁신 전략": {
                "블루오션 전략": "새로운 시장 창출과 차별화 전략",
                "디스럽티브 이노베이션": "시장 파괴적 혁신 전략",
                "린 스타트업": "최소 실행 가능 제품과 빠른 시장 검증"
            },
            "운영 전략": {
                "비즈니스 모델 캔버스": "비즈니스 모델의 9가지 핵심 요소 분석",
                "제약이론": "시스템의 제약 요인 분석과 개선",
                "밸류 체인": "가치 사슬 분석과 최적화"
            }
        }
        ...
        # 프레임워크가 선택되지 않은 경우 AI가 자동 선택
        if not selected_frameworks:
            st.info("AI가 키워드에 적합한 프레임워크를 자동으로 선정합니다.")
            
            # 키워드별 적합한 프레임워크 매핑
            keyword_frameworks = {
                "가치 창조": ["블루오션 전략", "비즈니스 모델 캔버스", "디스럽티브 이노베이션"],
                "마케팅": ["SWOT 분석", "마이클 포터의 경쟁전략", "협력적 경쟁관계"],
                "세일즈": ["마이클 포터의 경쟁전략", "협력적 경쟁관계", "린 스타트업"],
                "가치 전달": ["비즈니스 모델 캔버스", "린 스타트업", "제약이론"],
                "재무": ["SWOT 분석", "비즈니스 모델 캔버스", "제약이론"],
                "기타": ["SWOT 분석", "비즈니스 모델 캔버스", "마이클 포터의 경쟁전략"]
            }
            
            # 선택된 키워드에 따른 프레임워크 자동 선택
            selected_frameworks = keyword_frameworks.get(analysis_keyword, keyword_frameworks["기타"])
            
            if debug_mode:
                st.write(f"AI 선정 프레임워크: {selected_frameworks}")
        
        # 선택된 프레임워크 표시
        if selected_frameworks:
            st.success(f"선택된 프레임워크: {', '.join(selected_frameworks)}")
        ...
        with st.spinner("AI 에이전트들이 전략을 생성하고 있습니다..."):
            try:
                ...
                framework_experts = {
                    ...
                    "마이클 포터의 경쟁전략": "산업 구조와 경쟁 전략 전문가",
                    "비즈니스 모델 캔버스": "비즈니스 모델 혁신 전문가",
                    "협력적 경쟁관계": "협력적 경쟁 전략 전문가",
                    "린 스타트업": "린 방법론 전문가",
                ...
                for framework in selected_frameworks:
                    if framework in framework_experts:
                        agent = Agent(
                            ...
                            backstory=f"""당신은 {framework_experts[framework]}입니다. 
                            해당 프레임워크를 활용한 수많은 프로젝트 경험이 있으며, 
                            특히 {framework}의 핵심 개념과 방법론을 실제 비즈니스에 적용하는 전문성을 보유하고 있습니다.""",
                            verbose=debug_mode,
                            llm=llm
                        )
                        agents.append(agent)
