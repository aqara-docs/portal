...
def ai_generate_fable(text, fable_type, model_name, extra_prompt=None):
    ...
    fable_instructions = {
        ...
        "예수님과 12사도": (
            "아래 요약 내용을 바탕으로 예수님과 12사도가 등장하는 1분 30초 분량의 대화체 우화를 만들어 주세요. "
            "예수님의 가르침 방식을 따라 비유와 실제 삶의 예시를 통해 영적, 도덕적 교훈을 전달하는 형식으로 작성해 주세요. "
...
"""
            # 각 우화 스타일별 시스템 프롬프트
            system_prompts = {
                "예수님과 12사도": "당신은 성경의 예수님과 12사도의 대화를 구성하는 전문가입니다. 예수님의 가르침 방식을 따라 비유와 실제 삶의 예시를 통해 1분 30초 분량의 깊이 있는 대화를 구성하고, 제자들의 다양한 성격과 관점이 드러나도록 하며, 마지막에 명언을 한글과 영어로 제시합니다.",
                "하모니아": (
                    "당신은 동서양의 위대한 사상가들의 대화를 구성하는 전문가입니다. "
                    "다음 각 그룹에서 랜덤하게 1명씩 선택하여 3분 분량의 심오하고 통찰력 있는 대화를 구성합니다:\n"
                    "1. 4대 성인 중 1명 (예수님, 부처님, 공자님, 소크라테스님)\n"
...
def text_to_speech_fable(text, fable_type):
    ...
    try:
        ...
        voice_settings = {
            "예수님과 12사도": {"voice": "onyx", "speed": 0.85},  # 경건하고 깊이 있는 목소리
...
def main():
    ...
    with tab1:
        ...
        book_title = st.selectbox(
            "책 제목",
            ["Built to Last", "퍼스널 MBA", "레이달리오의 원칙", "Good to Great"],
            index=3
        )
    ...
    with tab4:
        ...
        with col1:
            content_type_filter = st.selectbox(
                "콘텐츠 유형", 
                ["전체", "요약", "적용파일", "우화"],
                key="playback_content_type"
            )
        with col2:
            # 책 제목 목록 가져오기
            all_records = get_reading_discussion_records()
            book_titles = ["전체"] + list(set([record['book_title'] for record in all_records if record['book_title']]))
            book_title_filter = st.selectbox(
                "책 제목",
                book_titles,
                key="playback_book_title"
            )
        
        # 필터링된 데이터 가져오기
        content_type_param = None if content_type_filter == "전체" else content_type_filter.lower()
        content_type_param = "summary" if content_type_param == "요약" else content_type_param
        content_type_param = "application" if content_type_param == "적용파일" else content_type_param
        content_type_param = "fable" if content_type_param == "우화" else content_type_param
        
        book_title_param = None if book_title_filter == "전체" else book_title_filter
        
        filtered_records = get_reading_discussion_records(content_type_param, book_title_param)
