...
def ai_summarize(text, model_name):
    summary_instruction = (
...
def discussion_order_tab():
    # 캐릭터/상수 (함수 최상단에 위치)
    CHARACTERS = {"현철님": "🧙‍♂️ 지혜의 현자", "창환님": "🚀 미래의 선구자", "성범님": "🎯 통찰의 대가", "상현님": "🎭 질문의 예술가", "성일님": "💡 혁신의 아이콘", "경호님": "🦾 전략의 마에스트로"}
    VALUES = [
                ...
                with st.spinner("저장 중입니다..."):
                    try:
                        if uploaded_file:
                            ...
                            summary = ai_summarize(raw_text, st.session_state.selected_model)
        ...
        with subtab2:
            ...
            if applications:
                ...
                if st.button("AI 요약", key="app_ai_summary"):
                    with st.spinner("AI가 적용 파일을 분석 중입니다..."):
                        try:
                            ai_app_analysis = ai_analyze_application(selected_application['content'], st.session_state.selected_model, extra_prompt_app)
                            st.session_state['ai_app_summary_result'] = ai_app_analysis
                        except Exception as e:
                            st.error(f"AI 분석 중 오류: {str(e)}")
                if st.session_state['ai_app_summary_result']:
                    st.markdown("### AI 분석 결과 (적합성/개선점/실행 제안)")
                    # 섹션별로 bold heading, 아래는 bullet point로 출력
                    ai_result = st.session_state['ai_app_summary_result']
                    import re
                    section_titles = ['적합성', '개선점', '실질적 실행 제안']
                    # 각 항목별로 제목이 이미 포함되어 있을 수 있으니, 제목을 제거하고 내용만 추출
                    # 1. 적합성 ...\n2. 개선점 ...\n3. 실질적 실행 제안 ...
                    # 정규식으로 "숫자. 제목" 패턴을 찾아 분리
                    pattern = r'\n?\s*\d+\.\s*([가-힣a-zA-Z ()]+)\n?'
                    splits = list(re.finditer(pattern, ai_result))
                    sections = []
                    for i, match in enumerate(splits):
                        start = match.end()
                        end = splits[i+1].start() if i+1 < len(splits) else len(ai_result)
                        section_text = ai_result[start:end].strip()
                        # 만약 bullet point가 없고 제목이 반복된다면, 제목 줄 제거
                        lines = section_text.split('\n')
                        if lines and lines[0].replace(':','').replace(' ','') in section_titles:
                            section_text = '\n'.join(lines[1:]).strip()
                        sections.append(section_text)
                    for i, section in enumerate(sections[:3]):
                        st.markdown(f"**{section_titles[i]}**")
                        st.markdown(format_bullet_points(section.strip()))
                if st.button("음성 생성", key="app_tts"):
                    with st.spinner("음성을 생성하고 있습니다..."):
                        try:
                            closing_ment = f"다음 시간에는 {next_topic if next_topic else '다음 주제'}에 대한 독서 토론을 진행할 예정입니다."
                            # 오프닝 멘트와 AI 분석 결과(적합성/개선점/실행 제안)만 포함
                            tts_text = f"즐거운 독서토론이 되셨는지요? 적용 파일에 대한 AI 분석 결과를 요약해 드리겠습니다.\n"
                            ai_result = st.session_state.get('ai_app_summary_result')
                            if ai_result:
                                import re
                                section_titles = ['적합성', '개선점', '실질적 실행 제안']
                                pattern = r'\n?\s*\d+\.\s*([가-힣a-zA-Z ()]+)\n?'
...
def ai_analyze_application(text, model_name, extra_prompt=None):
    prompt = (
        "아래 적용 파일 내용을 비즈니스 관점에서 분석해 주세요. 각 항목별로 bullet point(•)로, '~함' 형태의 간결한 한글로 정리해 주세요. 불필요한 설명 없이 핵심만 제시하세요. 반드시 존댓말(~합니다) 대신 '~함' 형태로 작성하세요.\n"
        "1. 적합성(타당성): 이 적용 내용이 실제 업무/비즈니스 환경에 적합한지 평가\n"
        "2. 개선점: 보완하거나 발전시킬 부분이 있다면 구체적으로 제안\n"
        "3. 실질적 실행 제안: 실제로 현장에서 바로 적용할 수 있는 구체적 실행 방안 1~2개 제시\n"
        "각 항목은 2~3개 bullet point로, '~함' 형태의 간결체로 작성하세요."
    )
    if model_name.startswith('claude'):
        client = ChatAnthropic(model=model_name, api_key=os.getenv('ANTHROPIC_API_KEY'), temperature=0.3, max_tokens=600)
        prompt_full = f"{prompt}\n---\n{text}"
        if extra_prompt and extra_prompt.strip():
            prompt_full += f"\n[참고 내용]\n{extra_prompt.strip()}\n"
        response = client.invoke([
            {"role": "system", "content": "당신은 비즈니스 적용 평가 전문가입니다. 반드시 각 항목별로 bullet point(•)로, '~함' 형태의 간결체로 작성합니다. 존댓말(~합니다) 금지. 불필요한 설명 금지."},
