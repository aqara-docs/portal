    ...
    if files:
        ...
        if selected_file:
            ...
            if material_type == "적용" and analysis_keyword:
                ...
                if st.button("🤖 AI 의견 듣기"):
                    with st.spinner("AI가 의견을 생성하고 있습니다..."):
                        display_ai_opinion(selected_file['content'])
    else:
...
def generate_business_opinion(key_point):
    """비즈니스 관점에서의 AI 의견 생성"""
    client = OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
    
    prompt = f"""
    다음 독서 토론 내용에 대해 비즈니스 전문가의 입장에서 의견을 제시해주세요:
    "{key_point}"
    
    - "오늘 독서 토론 내용 중..."으로 시작하여
    - 해당 내용이 비즈니스에 어떤 의미가 있는지
    - 어떻게 실제로 적용해볼 수 있는지
    - 구체적인 실행 방안까지 자연스럽게 설명해주세요
    
    1000자 내외로 자세히 설명해주세요.
    """
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "당신은 실무 경험이 풍부한 비즈니스 전문가입니다. 이론보다는 실제 적용 방안에 중점을 두어 조언합니다."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
        ...
        return "오늘 독서 토론 내용 중 이 부분이 매우 중요한 시사점을 제공합니다. 이를 실제 업무에 적용하려면 구체적인 실행 계획과 단계별 접근이 필요할 것 같습니다. 먼저 팀 내에서 파일럿 프로젝트로 시작해보는 것을 추천드립니다. 2-3개월간의 소규모 테스트를 통해 효과를 검증하고, 이후 전사적으로 확대 적용하면 리스크를 최소화하면서 변화를 이끌어낼 수 있을 것입니다."

def display_ai_opinion(summary_text):
    """AI의 의견을 생성하고 표시하는 함수"""
    st.markdown("""
    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 5px solid #4CAF50;">
        <h4 style="color: #2E7D32;">🤖 비즈니스 전문가의 의견</h4>
    ...
    if best_sentence:
        opinion = generate_business_opinion(best_sentence)
        st.write(opinion)  # st.write를 사용하여 일반 텍스트로 표시
