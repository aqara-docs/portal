...
def get_llm_models():
    ...
    models = {
        ...
        "Ollama (로컬)": {
            "DeepSeek-70B": "deepseek-r1:70b",  # 42GB - 가장 큰 모델
            "DeepSeek-32B": "deepseek-r1:32b",  # 19GB
            "DeepSeek-14B": "deepseek-r1:14b",  # 9.0GB
            "Phi-4": "phi4:latest",             # 9.1GB
            "Gemma2": "gemma2:latest",          # 5.4GB
            "Llama3.1": "llama3.1:latest",      # 4.9GB
            "Mistral": "mistral:latest",        # 4.1GB
            "Llama2": "llama2:latest",          # 3.8GB
...
def create_llm(provider, model_name):
    ...
    if provider == "OpenAI":
        ...
        # CrewAI와 호환성 문제로 인해 OpenAI 모델 사용 권장
        st.warning("CrewAI와 Ollama의 호환성 문제로 인해 OpenAI 모델 사용을 권장합니다.")
        ...
        class OllamaWrapper:
            def __init__(self, model_name):
                self.model_name = "ollama/mistral"  # 항상 고정된 모델명 사용
                self._model_name = model_name  # 실제 모델명 저장
                
            def complete(self, prompt):
                try:
                    with st.spinner(f"Ollama ({self._model_name}) 생성 중..."):
                        # litellm을 통해 Ollama 호출
                        response = litellm.completion(
                            model="ollama/mistral",  # 매핑된 모델명 사용
