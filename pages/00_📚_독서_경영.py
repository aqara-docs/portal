...
"""
        ...
        language_formats = {
            "한국어": "한글 명언: \"명언 내용\"",
            "한국어+영어": "한글 명언: \"명언 내용\"\n영문 명언: \"Quote content\"",
            "한국어+영어+중국어": "한글 명언: \"명언 내용\"\n영문 명언: \"Quote content\"\n중국어 명언: \"中文名言\"\n한어병음: \"zhōng wén míng yán\"",
            "한국어+영어+중국어+일본어": "한글 명언: \"명언 내용\"\n영문 명언: \"Quote content\"\n중국어 명언: \"中文名言\"\n한어병음: \"zhōng wén míng yán\"\n일본어 명언: \"日本語の名言\"\n카타카나: \"ニホンゴノメイゲン\""
        }
...
def main():
    ...
    with col1:
        # 책 제목 선택 (기본값: "AX 100배의 법칙")
        default_book = "AX 100배의 법칙"
        
        # 책 제목 목록을 우선순위에 따라 정렬
        priority_books = ["AX 100배의 법칙", "성장의 묘약"]
        sorted_book_titles = []
        
        # 우선순위 책들을 먼저 추가
        for priority_book in priority_books:
            if priority_book in book_titles:
                sorted_book_titles.append(priority_book)
        
        # 나머지 책들을 알파벳 순으로 추가
        remaining_books = [book for book in book_titles if book not in priority_books]
        remaining_books.sort()
        sorted_book_titles.extend(remaining_books)
        
        if default_book not in sorted_book_titles and sorted_book_titles:
            default_book = sorted_book_titles[0]
        
        selected_book = st.selectbox(
            "📖 책 제목으로 검색",
            ["전체"] + sorted_book_titles,
            index=0 if default_book not in sorted_book_titles else sorted_book_titles.index(default_book) + 1,
            key="book_search"
        )
    ...
    # 필터링된 데이터 가져오기
    filtered_records = get_reading_discussion_records(
        content_type=content_type_filter if content_type_filter != "전체" else None,
        book_title=selected_book if selected_book != "전체" else None
    )
    
    # 레코드 수 제한
    filtered_records = filtered_records[:record_count]
    
    # 검색 결과 요약 표시
    if selected_book != "전체" or content_type_filter != "전체":
        st.info(f"🔍 검색 결과: {len(filtered_records)}개의 기록을 찾았습니다.")
        if selected_book != "전체":
            st.write(f"📖 책 제목: {selected_book}")
        if content_type_filter != "전체":
            st.write(f"📝 콘텐츠 타입: {content_type_filter}")
    ...
    if filtered_records:
        ...
        if selected_book != "전체" or content_type_filter != "전체":
            st.warning(f"검색 조건에 맞는 기록이 없습니다.")
            st.write(f"📖 선택된 책: {selected_book}")
            st.write(f"📝 선택된 콘텐츠 타입: {content_type_filter}")
        else:
    ...
    st.markdown("""
    **💡 사용 팁:**
    - **🔍 검색 기능**: 책 제목과 콘텐츠 타입으로 필터링할 수 있습니다
    - **📖 기본 검색**: "AX 100배의 법칙"이 기본으로 선택됩니다
    - **📝 콘텐츠 타입**: summary(요약), application(적용), fable(우화) 중 선택
    - **탭 구분**: "AI 핵심 요약" 및 "AI 요약 및 총평" 탭의 내용이 실제 음성으로 변환된 텍스트입니다
    - **다운로드**: 각 음성 파일을 다운로드할 수 있습니다
    - **🌍 언어 선택**: 4가지 언어 옵션을 선택할 수 있습니다
      - **한국어+영어** (기본): 한국어와 영어로 명언 생성
      - **한국어**: 한국어로만 명언 생성
      - **한국어+영어+중국어**: 한국어, 영어, 중국어로 명언 생성 (중국어에 한어병음 포함)
      - **한국어+영어+중국어+일본어**: 한국어, 영어, 중국어, 일본어로 명언 생성 (중국어에 한어병음, 일본어에 카타카나 포함)
