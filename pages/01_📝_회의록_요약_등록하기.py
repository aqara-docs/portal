...
def main():
    ...
    with tab1:
        ...
        # 안내 문구 추가
        st.info("회의 녹음 파일, 텍스트 파일, 또는 아래 텍스트박스 중 하나만 입력해도 AI 요약이 가능합니다. 텍스트박스에 입력된 내용이 있으면 파일보다 우선 사용됩니다.")
        
        # 직접 텍스트 입력용 텍스트박스 추가
        direct_text_input = st.text_area(
            "직접 회의록 텍스트 입력(선택)",
            value="",
            placeholder="여기에 회의 내용을 직접 입력할 수 있습니다. 파일 업로드 대신 사용할 수 있습니다.",
            height=200,
            key="direct_text_input_box"
        )
            ...
            if title and participants:
                ...
                try:
                    ...
                    # 2순위: 파일 업로드
                        ...
                        analysis_status.error("❗ 회의록 텍스트를 직접 입력하거나 파일을 업로드해 주세요.")
                        st.session_state.analysis_started = False
                        return
                    if text:
