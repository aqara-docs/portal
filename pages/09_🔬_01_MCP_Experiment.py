...
if 'local_server_type' not in st.session_state:
    st.session_state['local_server_type'] = 'mysql'
...
def start_mcp_server(server_type, port):
    ...
    try:
        ...
        if server_type == "mysql":
            script_path = os.path.join(servers_dir, "mcp_mysql_server.py")
        elif server_type == "firecrawl":
            script_path = os.path.join(servers_dir, "mcp_firecrawl_server.py")
        elif server_type == "perplexity":
            script_path = os.path.join(servers_dir, "mcp_perplexity_server.py")
        else:
            return False, f"지원하지 않는 서버 유형입니다: {server_type}"
        
        # 서버 스크립트 파일이 없으면 생성
        if not os.path.exists(script_path):
            create_server_script(server_type, script_path)
        ...
        try:
            ...
            stop_mcp_server()
            return False, f"서버 시작 실패: {str(e)}"
        
        return True, f"{server_type.upper()} MCP 서버가 시작되었습니다. 잠시 후 연결을 시도하세요."
...
def create_server_script(server_type, script_path):
    """서버 스크립트 파일 생성"""
    if server_type == "mysql":
        script_content = """
import sys
...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/status":
            ...
            self.wfile.write(json.dumps(status).encode())
            print(f"Status request received and responded with: {json.dumps(status)}")
        else:
            self._handle_error(404, "Not found")
            print(f"404 error for path: {self.path}")
    ...
    def do_POST(self):
        if self.path == "/execute":
            ...
            print(f"Received request: {json.dumps(request)}")
            ...
            if not api_key:
                return self._handle_error(400, "FireCrawl API key is required")
            
            if tool == "search":
                return self.handle_search(params, api_key)
            elif tool == "crawl":
                return self.handle_crawl(params, api_key)
            else:
                return self._handle_error(400, f"Unknown tool: {tool}")
    
    def handle_search(self, params, api_key):
        if "query" not in params:
            return self._handle_error(400, "Missing query parameter")
        ...
        try:
            ...
            search_url = "https://api.firecrawl.co/api/v1/search"
            ...
            self._set_headers()
            result = {"results": response.json()}
            print(f"Sending search results: {json.dumps(result)}")
            self.wfile.write(json.dumps(result).encode())
            
        except requests.exceptions.RequestException as e:
            error_msg = f"FireCrawl search error: {str(e)}"
            print(f"Error in search: {error_msg}")
            return self._handle_error(500, error_msg)
    
    def handle_crawl(self, params, api_key):
        ...
        try:
            ...
            crawl_url = "https://api.firecrawl.co/api/v1/crawl"
            ...
            print(f"Sending crawl request to {crawl_url}")
            response = requests.post(crawl_url, headers=headers, json=crawl_data)
            ...
            self._set_headers()
            result = {"results": response.json()}
            print(f"Sending crawl results: {json.dumps(result)}")
            self.wfile.write(json.dumps(result).encode())
            
        except requests.exceptions.RequestException as e:
            error_msg = f"FireCrawl crawl error: {str(e)}"
            print(f"Error in crawl: {error_msg}")
            return self._handle_error(500, error_msg)
...
def run_server(port):
    ...
    for attempt in range(max_retries):
        try:
            print(f"Starting FireCrawl MCP server on port {port} (attempt {attempt + 1}/{max_retries})")
            httpd = HTTPServer(server_address, MCPHandler)
            print(f"FireCrawl MCP server is running on port {port}")
            httpd.serve_forever()
            break
        except OSError as e:
...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def handle_search(self, params):
        ...
        try:
            ...
            if not api_key:
                return self._handle_error(400, "Perplexity API key is required")
            ...
            return self._handle_error(500, f"Perplexity API error: {str(e)}")
...
def call_claude_api(user_input, model="claude-3-5-sonnet-20240620"):
    ...
    try:
        ...
        return response.json()["content"][0]["text"].strip(), None
    except Exception as e:
...
def initialize_mcp_server(config_data):
    try:
        ...
        if isinstance(config_data, dict) and "mcpServers" in config_data and config_data["mcpServers"]:
            ...
            # command와 args가 있는 경우 추가 확인
            if "command" in server_config and "args" in server_config:
                args_str = " ".join(str(arg) for arg in server_config["args"])
                if "firecrawl" in args_str.lower() or "mcp-firecrawl-server" in args_str:
                    server_type = "firecrawl"
                elif "perplexity" in args_str.lower() or "mcp-perplexity-server" in args_str:
                    server_type = "perplexity"
            
            st.info(f"### MCP 서버 설정 감지됨 (유형: {server_type})")
            st.code(f"서버 이름: {server_name}\n명령어: {server_config.get('command', '')}\n인자: {' '.join(str(arg) for arg in server_config.get('args', []))}")
            ...
            config_data["serverType"] = server_type
            config_data["serverUrl"] = f"http://{server_url}"
        ...
        if isinstance(config_data, str):
            ...
            if "mcpServers" in config_data:
                ...
                if "firecrawl" in server_name.lower():
                    ...
                    server_type = "perplexity"
                elif "mysql" in server_name.lower():
                    server_type = "mysql"
...
with st.sidebar:
    ...
    # MySQL settings in sidebar (without nested columns)
    if local_server_type == "mysql":
        st.sidebar.markdown("### MySQL 연결 설정")
        mysql_userid = st.sidebar.text_input("MySQL 사용자 ID", value=os.getenv('SQL_USER', ''))
        mysql_password = st.sidebar.text_input("MySQL 비밀번호", type="password", value=os.getenv('SQL_PASSWORD', ''))
        mysql_database = st.sidebar.text_input("데이터베이스", value=os.getenv('SQL_DATABASE_NEWBIZ', ''))
        
        if not mysql_userid or not mysql_password:
            st.sidebar.warning("MySQL 연결을 위해 사용자 ID와 비밀번호를 입력해주세요.")
        ...
        try:
            if json_config:
                ...
                save_to_db = st.checkbox(
                    "이 설정을 DB에 저장",
                    value=True,
                    help="설정을 DB에 저장하여 나중에 다시 사용할 수 있습니다."
                )
                ...
                if st.button("JSON으로 연결"):
                    ...
                    with st.spinner("Connecting to MCP server..."):
                        ...
                        if client:
                            # 서버 타입을 사이드바에서 선택된 값으로 설정
                            server_type = st.session_state.get('local_server_type', 'mysql')
                            ...
                            if save_to_db:
                                try:
                                    # 서버 URL 결정
                                    server_url = config_data.get("serverUrl", client.server_url)
                                    if not server_url.startswith(("http://", "https://")):
                                    ...
                                    # 디버그 정보 출력
                                    st.write("저장할 설정 정보:")
                                    st.write(f"- 이름: {json_config_name}")
                                    st.write(f"- 타입: {server_type}")
                                    st.write(f"- URL: {server_url}")
                                    
                                    # 설정 저장 시도
                                    success, save_message = save_mcp_server_config(
                                        json_config_name,
                                        server_type,
                                        server_url,
                                        config_json_str
                                    ...
                                    if success:
                                        st.success(f"서버 연결 및 설정 저장 완료\n- 연결: {message}\n- 저장: {save_message}")
                                    else:
                                        st.warning(f"서버 연결은 성공했지만 설정 저장 실패\n- 연결: {message}\n- 저장 오류: {save_message}")
                                except Exception as e:
        ...
        if st.button("연결", key="quick_connect"):
            ...
            with st.spinner("Connecting to MCP server..."):
                ...
                if client:
                    ...
                    # DB에 저장할지 물어보기
                    if st.checkbox("이 설정을 DB에 저장하시겠습니까?", value=True, key="quick_save_db"):
                        success, save_message = save_mcp_server_config(
                            config_name,
                            server_type,
                            server_url,
