...
def initialize_session_state():
    ...
    # 기본 세션 상태 구조 정의
    default_state = {
        "current_analysis_id": None,  # 현재 선택된 분석 ID
        "saved_analyses": [],         # 저장된 분석 목록
        "analysis_results": {},       # 현재 분석 결과
        "active_agents": {            # 활성화된 에이전트
            'financial_agent': True,
            'market_agent': True,
            'tech_agent': True,
            'risk_agent': True,
    ...
    # analysis_state가 없으면 새로 생성
    if "analysis_state" not in st.session_state:
        st.session_state.analysis_state = default_state
    else:
        # 기존 세션 상태와 새로운 구조 병합
        ...
        # active_agents가 있는 경우 누락된 에이전트 추가
        if "active_agents" in current_state:
            for agent, default_value in default_state["active_agents"].items():
                if agent not in current_state["active_agents"]:
                    current_state["active_agents"][agent] = default_value
        
        # save_status가 없는 경우 추가
        if "save_status" not in current_state:
            current_state["save_status"] = default_state["save_status"]
        
        # 기존 메시지 상태를 새로운 구조로 마이그레이션
        if "save_message" in current_state and "save_message_type" in current_state:
            current_state["save_status"] = {
                "success": current_state.get("save_success", False),
                "message": current_state["save_message"],
                "message_type": current_state["save_message_type"]
            }
            # 기존 필드 제거
            for key in ["save_message", "save_message_type", "save_success"]:
                if key in current_state:
                    del current_state[key]
        
        st.session_state.analysis_state = current_state

def update_session_state(key, value, message=None, message_type="info"):
    """세션 상태 업데이트 헬퍼 함수"""
    if key in st.session_state.analysis_state:
        st.session_state.analysis_state[key] = value
        if message:
            st.session_state.analysis_state["save_message"] = message
            st.session_state.analysis_state["save_message_type"] = message_type
...
def save_analysis_to_db(query, analysis_results, update_existing=False):
    ...
    try:
        conn = connect_to_db()
        if not conn:
            st.session_state.analysis_state["save_status"] = {
                "success": False,
        ...
        if update_existing and analysis_results.get('metadata', {}).get('original_id'):
            # 기존 분석 결과 업데이트
            cursor.execute("""
                ...
                WHERE id = %s
            """, (query, json_results, analysis_results['metadata']['original_id']))
            
            if cursor.rowcount == 0:
                # 업데이트할 레코드가 없으면 새로 삽입
                cursor.execute("""
                    INSERT INTO mcp_analysis_results 
                    (query, analysis_result)
                    VALUES (%s, %s)
        ...
        # 세션 상태 업데이트
        st.session_state.analysis_state["current_analysis_id"] = new_id
        st.session_state.analysis_state["analysis_results"] = analysis_results
        st.session_state.analysis_state["save_status"] = {
            "success": True,
            "message": "기존 분석 결과를 업데이트했습니다." if update_existing else "새 분석 결과를 저장했습니다.",
            "message_type": "success"
        }
        
        # 저장된 분석 목록 새로고침
        st.session_state.analysis_state["saved_analyses"] = load_saved_analyses()
...
def load_saved_analyses():
    ...
    try:
        ...
        for result in results:
            ...
            # analysis_result가 None인 경우 빈 딕셔너리로 초기화
            if result['analysis_result'] is None:
                result['analysis_result'] = '{}'
            # analysis_result가 문자열인 경우 그대로 유지
            elif isinstance(result['analysis_result'], str):
                pass
            # 그 외의 경우 JSON 문자열로 변환
            else:
                result['analysis_result'] = json.dumps(result['analysis_result'], ensure_ascii=False)
...
def main():
    ...
    # 저장 상태 메시지 표시 (안전하게 접근)
    save_status = st.session_state.analysis_state.get("save_status", {
        "success": False,
        "message": "",
        "message_type": "info"
    })
    
    if save_status.get("message"):
        if save_status["message_type"] == "success":
            st.success(save_status["message"])
        elif save_status["message_type"] == "error":
            st.error(save_status["message"])
        else:
            st.info(save_status["message"])
        # 메시지 표시 후 초기화
        st.session_state.analysis_state["save_status"]["message"] = ""

    # 저장된 분석 결과 목록 로드 (세션에 없을 경우에만)
    if not st.session_state.analysis_state.get("saved_analyses"):
        st.session_state.analysis_state["saved_analyses"] = load_saved_analyses()
    ...
    with tab2:
        ...
        with st.expander("🤖 AI 에이전트 설정", expanded=True):
            ...
            with col1:
                st.session_state.analysis_state["active_agents"]["financial_agent"] = st.checkbox(
                    "재무 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["financial_agent"]
                )
                st.session_state.analysis_state["active_agents"]["market_agent"] = st.checkbox(
                    "시장 분석가", 
                    value=st.session_state.analysis_state["active_agents"]["market_agent"]
                )
                st.session_state.analysis_state["active_agents"]["tech_agent"] = st.checkbox(
                    "기술 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["tech_agent"]
                )
            
            with col2:
                st.session_state.analysis_state["active_agents"]["risk_agent"] = st.checkbox(
                    "리스크 관리자", 
                    value=st.session_state.analysis_state["active_agents"]["risk_agent"]
                )
                st.session_state.analysis_state["active_agents"]["legal_agent"] = st.checkbox(
                    "법률 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["legal_agent"]
                )
                st.session_state.analysis_state["active_agents"]["hr_agent"] = st.checkbox(
                    "인사 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["hr_agent"]
                )
            
            with col3:
                st.session_state.analysis_state["active_agents"]["operation_agent"] = st.checkbox(
                    "운영 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["operation_agent"]
                )
                st.session_state.analysis_state["active_agents"]["strategy_agent"] = st.checkbox(
                    "전략 전문가", 
                    value=st.session_state.analysis_state["active_agents"]["strategy_agent"]
                )
                st.session_state.analysis_state["active_agents"]["integration_agent"] = st.checkbox(
                    "통합 매니저", 
                    value=True, 
                    disabled=True
                )
        ...
        try:
            if not st.session_state.analysis_state["saved_analyses"]:
                ...
                with col1:
                    selected_analysis = st.selectbox(
                        "상세 분석할 데이터 선택",
                        st.session_state.analysis_state["saved_analyses"],
                        format_func=lambda x: f"{x['query'][:100]}... ({x['created_at']})",
                        key="analysis_selector"
                    )
                    if selected_analysis:
                        st.session_state.analysis_state["current_analysis_id"] = selected_analysis['id']
                
                with col2:
                    if st.button("🗑️ 선택한 분석 삭제", type="secondary", help="선택한 분석을 데이터베이스에서 삭제합니다"):
                        if delete_saved_analysis(selected_analysis['id']):
                            st.session_state.analysis_state["save_status"] = {
                                "success": True,
                            ...
                            # 저장된 분석 목록 새로고침
                            st.session_state.analysis_state["saved_analyses"] = load_saved_analyses()
                            st.session_state.analysis_state["current_analysis_id"] = None
                            st.rerun()
                        else:
                            st.session_state.analysis_state["save_status"] = {
                                "success": False,
                ...
                if selected_analysis:
                    ...
                    if st.button("🔍 상세 분석 실행", type="primary"):
                        ...
                        with status_placeholder:
                            st.info("🔄 상세 분석을 시작합니다...")

                        try:
                            # 원본 분석과 지시사항을 결합한 쿼리 생성
                            combined_query = f"""
                            ...
                            # 상세 분석 실행
                            results = asyncio.run(process_multi_agent_query(
                                combined_query,
                                text_placeholder,
                                tool_placeholder,
                                active_agents=st.session_state.analysis_state["active_agents"],
                            ...
                            if results and not (isinstance(results, dict) and "error" in results and len(results) == 1):
                                with result_area:
                                    ...
                                    # 저장 버튼을 탭 외부로 이동 (분석 결과가 있을 때만 표시)
                                    st.markdown("---")
                                    ...
                                    with col_save1:
                                        save_as_new = st.checkbox("새 분석으로 저장", value=True,
                                                                ...
                                                                key="save_as_new_checkbox")
                                    with col_save2:
                                        if st.button("💾 모든 분석 결과 저장", type="primary", key="save_all_results"):
                                            # 저장할 데이터 준비
                                            save_query = f"{selected_analysis['query']} [추가 분석: {analysis_instruction}]"
                                            save_results = {
                                                'integration_agent': results.get('integration_agent', {}),
                                                'individual_agents': {
                                                    agent: results.get(agent, {})
                                                    for agent in st.session_state.analysis_state["active_agents"]
                                                    if agent in results and agent != 'integration_agent'
                                                },
                                                'metadata': {
                                                    'active_agents': st.session_state.analysis_state["active_agents"],
                                                    'analysis_instruction': analysis_instruction,
                                                    'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                                    'original_id': selected_analysis['id'] if not save_as_new else None
                                                }
                                            }
                                            
                                            if save_analysis_to_db(save_query, save_results, not save_as_new):
                                                st.rerun()
                                    ...
                                    with main_tabs[1]:
                                        ...
                                        filtered_agents = {
                                            ...
                                            if agent in st.session_state.analysis_state["active_agents"] 
                                            and st.session_state.analysis_state["active_agents"][agent]
                                            and agent in results 
                                            and agent != "integration_agent"
                                        }
